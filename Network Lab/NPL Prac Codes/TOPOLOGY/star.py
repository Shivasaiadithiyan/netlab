...
class StarProtocol(protocol.Protocol):
    def __init__(self, factory):
        self.factory = (
            factory  # factory that stores all the clients connected to the server
        )
    ...
    def dataReceived(self, data):
        ...
        if not self.name:
            self.name = message
            print(
                self.name, " has connected to the server."
    ...
    def sendthroughServer(self, recipient, message):
        print(message)  # the message first goes to the server
        self.sendPrivateMessage(
            recipient, message
        )  # the message is then sent to the destination through the server
...
if __name__ == "__main__":
    ...
    print(
        "Server started. Listening on port 8000. Connect client by 'telnet localhost 8000'"
    )
    print(
        "Star topology -- Message first goes through hub (server), and then to client. No broadcast messages can be sent."
    )
    print(
        "Enter first message as name of user. Enter '@user: message' to send message to a particular client."
    )
    reactor.run()
