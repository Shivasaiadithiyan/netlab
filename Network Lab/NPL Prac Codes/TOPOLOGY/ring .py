...
class RingProtocol(protocol.Protocol):
    def __init__(self, factory):
        self.factory = (
            factory  # factory that stores the clients connected to the server
        )
                ...
                while sender_index != receiver_index:
                    ...
                    if self.factory.clients[sender_index] is None:
                        ...
                        self.transport.write(
                            "link failure. message cannot be sent".encode()
                        )
                        break

                    self.sendPrivateMessage(
                        self.factory.names[sender_index], private_message
                    )
...
if __name__ == "__main__":
    ...
    print(
        "Server started. Listening on port 8080. Connect client by 'telnet localhost 8000'. No broadcast message can be sent."
    )
    print("Messages are sent along right neighbors in topology.")
    print(
        """Enter first message as username to register. Enter '@username: message' to send message to another client."""
    )
    reactor.run()
